...
The best-fit Freundlich isotherm is shown alongside the experimental data in @fig-EtHex-ratio-comparison, fitted using linear least-squares methods with an R-squared value of 0.921. The fitted isotherm had a value for $n$ of $n = 2.1\pm1.1$.  The isotherm has previously been used to model adsorption of volatile organic molecules onto single-walled carbon nanotubes. A value for $n$ below $\sim$ 5 indicates the carbon nanotube network morphology used here has a relatively high maximum adsorption capacity. This results from using a nanotube network with a relatively high external surface area [@Agnihotri2005].
