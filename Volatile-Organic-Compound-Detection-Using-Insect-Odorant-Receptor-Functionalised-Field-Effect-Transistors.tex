...
\caption{\label{fig-FITC-SU8}A fluorescence image of a SU8-encapsulated
carbon nanotube device is shown in (a), while (b) shows the same channel
after modification with an solution of 1 mM Pyrene-PEG-FITC. A 0.35 s
exposure time and FITC filter were used for (a)-(b). The fluorescence
image in (c) shows an unencapsulated channel, while (d) shows the same
channel after exposure to 500 nM Cy3-tagged aptamer. A 10 s exposure
time and mCherry filter were used for (c)-(d). The fluorescence images
in (e) and (f) show carbon nanotube device channels pre-coated with a
thin layer of photoresist then submerged in fluorescent-tagged aptamer,
where the device in (f) was hardbaked at 200\(^\circ\)C for 1 hour
before exposure to aptamer. An mCherry filter and 30 s exposure time
were used for for (e)-(f).}
...
with the buffer and prevent sensing. Fluorescence microscopy was
therefore used as a tool to detect residue and test suitable residue
...
From comparing Figure~\ref{fig-aptamer-photoresist-2} and
