...
\autocite{LeMieux2008,Liu2013,Vobornik2023}. However, when we extract
...
and carbon nanotube surface both have a degree of roughness. To find the
surface roughness corresponding to each deposition type, silicon dioxide
substrates were modified using the same processes as in
Figure~\ref{fig-afm-morphology} but without carbon nanotubes present in
the solutions used. 2.5 \(\mu\)m \(\times\) 2.5 \(\mu\)m AFM images of
the resulting surfaces are shown in Figure~\ref{fig-afm-substrate}.
...
skewness. The skew is likely introduced by slight distortion of the
atomic force microscope image, as the roughness of a clean SiO\(_2\)
surface should follow a normal distribution \autocite{Velicky2015}.
Figure~\ref{fig-sio2-histogram} and
...
\(\alpha\) is set equal to zero \autocite{Azzalini1999}. The
correspondence between \(\xi\) and \(\omega\) for these distributions
but not \(\alpha\) further implies that the skewness is an imaging or
...
\autocite{Vobornik2023}. To determine the contribution of the
surfactant, we fit a skew-normal distribution to the curve, but only
letting \(\alpha\) vary with fixed values of \(\xi = 2.2\) nm and
\(\omega = 0.5\) nm. The result is shown in
...
to model the AFM histograms in Figure~\ref{fig-afm-morphology}. Using
the process discussed in Section~\ref{sec-histogram-analysis}, we
approximate the normal distributions corresponding to the silicon oxide
substrate and carbon nanotube network, shown in
