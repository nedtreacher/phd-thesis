...
histograms extracted from the AFM profile. Each histogram is fitted with
a linear combination of normal distributions. The component normal
...
contribute to surface roughness. Nanotubes can also undergo significant
compression from the AFM tip in tapping mode \autocite{Vobornik2023}.
...
\textbf{?@fig-afm-histograms} corresponds to the SiO\(_2\) substrate
...
\textbf{?@fig-afm-histograms}, we notice that there are two
...
our fit. The linear combination of distributions given by our initial
guess of parameters is shown by the green dotted line in
Figure~\ref{fig-scipy-gauss-fit}.

\begin{figure}

{\centering \includegraphics[width=0.6\textwidth,height=\textheight]{./figures/app2/Ned_NTQ5D12_pristine_00081_20210423_histogram_initialguess.png}

}

\caption{\label{fig-scipy-gauss-fit}Comparing the initial guess and the
scipy.optimise fit to the input atomic force microscope histogram}
...
parameters which gave an R-squared value of 0.96 or greater. The linear
combination of distributions corresponding to these optimised fitting
parameters is shown with the orange dotted line in
Figure~\ref{fig-scipy-gauss-fit}. By calculating the area under each of
the normal distributions found from the fit, we can find the proportion
of the surface covered by carbon nanotube bundles relative. The code
also allows for discretely binning continuous data from fitted normal
distributions and examining the proportion of counts above or below a
particular height.
