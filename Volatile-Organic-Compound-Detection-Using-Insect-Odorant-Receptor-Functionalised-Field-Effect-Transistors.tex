...
\frontmatter
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[enhanced, borderline west={3pt}{0pt}{shadecolor}, frame hidden, interior hidden, breakable, boxrule=0pt, sharp corners]}{\end{tcolorbox}}\fi
...
wider 20 \(\mu\)m encapsulation mask discussed in
\textbf{?@sec-encapsulation} made this possible. These images are shown
in Figure~\ref{fig-working-OR22a-AFM}. The position of nanodiscs
relative to the carbon nanotubes is less clearly seen on the denser
network morphology used here than on the more bundled morphology used by
...
in Figure~\ref{fig-working-OR22a-AFM} (b) is \(5.8\pm1.3\) nm. The
substrate mask used to find the average height is shown in
Figure~\ref{fig-working-OR22a-masks} (a), with this background having an
average height of \(4.6\pm0.4\) nm. The substrate mask of the
OR22a-functionalised device shown in Figure~\ref{fig-working-OR22a-AFM}
(c) is shown in Figure~\ref{fig-working-OR22a-masks} (b). Using this
mask, it was found that the substrate height had an average height of
\(5.7 \pm 0.6\) nm. The maximum height of the vertically-stacked carbon
nanotubes and OR22a nanodiscs was found to be \$\sim\$20 nm by
subtracting the height of the substrate from the total height of the AFM
image in Figure~\ref{fig-working-OR22a-AFM} (c). The average height of
the carbon nanotubes can then be subtracted from \$\sim\$20 nm to obtain
the height range for the OR22a aggregates, \(10-14\) nm, where
\$\sim\$10 nm is the minimum nanodisc height
\autocite{Nath2007,Bayburt2010,Murugathas2020}.

Note that the height range estimate given here assumes that nanodiscs do
not lie on junctions between nanotubes, which is unlikely to be the case
in practice. It also assumes that, due to the high density of the
network, that the nanodiscs predominantly sit on top of the carbon
nanotubes instead of directly adjacent to them on the substrate surface.
Height measurements of biological samples taken via AFM has been shown
to underestimate actual feature height by over 50\%
\autocite{Vobornik2023}. Despite the degree of uncertainty these
approximations introduce, it can still be reasonably concluded that the
breadth of these aggregations is well in excess of their height. While
the cross-sections of the largest features are up to 20 nanodiscs
across, they are a few nanodiscs high at most; certainly less than five,
and quite possibly not more than one.

The significant difference between the height and breadth of nanodiscs
indicates that \(-\) rather than clustering together in solution and
attaching at a single location as an aggregate, or nucleating out of
solution around a single attached nanodisc \(-\) the nanodiscs have
individually attached to a preferred site on the carbon nanotube
network. This preferred site may be at junctions between two large
carbon nanotubes, where there is a larger surface area available for
binding than on the sidewalls of a carbon nanotube bundle. It is also
possible that these sites are those that are particularly clean
contamination, either atmospheric or resulting from the deposition
...
network and not simply sticking to each other is a positive indicationr
