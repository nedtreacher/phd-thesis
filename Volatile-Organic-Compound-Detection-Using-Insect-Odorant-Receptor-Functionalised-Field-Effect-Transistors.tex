...
\autocite{Perez2015}. It has been argued that this label is unhelpfully
specific and a misrepresentation of what can be simply classed as a type
of Van Der Waals bonding \autocite{Martinez2012,Perez2015}. However, as
the use of the term is widespread in the literature, it is also used
here for ease of reference. Carbon nanotubes and graphene consist of a
network of carbon atoms attached to each other by sp\(^{2}\) hybrid
orbitals in a polycyclic structure. They are therefore able to strongly
interact with linker molecules with aromatic moieties, such as pyrene
\autocite{DiCrescenzo2014,Perez2015}.
...
been used to demonstrate successful functionalisation. However, until
recently there has been little justification provided for the selection
of a particular procedure. In fact, there is a notable lack of
systematic research into formation of pyrene-derivative monolayers on
graphene and other carbon nanomaterials, despite the wide use of this
chemistry for sensors the literature \autocite{Hinnemo2017,Zhen2018}.
This is surprising given that, for example, the sensitivity of aptamer
functionalised graphene devices is considered to be closely related to
the density of functionalised aptamer on the graphene channel
\autocite{Hao2020}. Detailed investigation of suitable PBASE
functionalisation parameters has only been undertaken for graphene-based
devices \autocite{Zhen2018,Hao2020}. Importantly, Hao \emph{et al.}
determine that acheiving a highly dense surface coverage of PBASE
results in more sensitive aptameric sensing \autocite{Hao2020}.
...
Figure~\ref{fig-pbase-nmr} compares the shapes of hydrogen nuclear
magnetic resonance (NMR) spectra of PBASE from each supplier when
dissolved in deuterated DMSO, alongside a blank deuterated DMSO
spectrum. We see both PBASE samples possess characteristic features
between \(2.1-2.2\) ppm, \(2.8-2.9\) ppm, and \(3.4-3.5\) ppm. The
chemical shift feature at 2.50 ppm represents the deuterated DMSO
...
The H\(_{2}\)O:DMSO ratio is 1:7 in the blank spectrum, but
approximatley \(\sim\) 1:3 in the provided samples, possibly indicating
...
storage time and environmental humidity during sample preparation may
have influenced this result \autocite{Lebel1962}. Other impurities are
also seen on both PBASE spectra, though their small size indicates they
make up only a small percentage of each sample.
...
\hypertarget{other-pyrene-based-linkers}{%
\subsection{Other Pyrene-Based
Linkers}\label{other-pyrene-based-linkers}}
