...
\frontmatter
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, frame hidden, enhanced, breakable, sharp corners, interior hidden, boxrule=0pt]}{\end{tcolorbox}}\fi
...
\autocite{Thanihaichelvan2018,Liu2013,Vobornik2023}. However, when we
...
backgated sweeps. Each sweep was taken from a separate device. Devices
with a 100 nm SiO\(_2\) layer were used for backgated measurements, and
devices with a 300 nm SiO\(_2\) layer were used for liquid gated
measurements.}
...
Figure Figure~\ref{fig-pristine-cnt-characteristics} displays
...
\autocite{Kauffman2008,JongYu2009,Derenskyi2014,Thanihaichelvan2018,Albarghouthi2022}.
Devices showed significantly less hysteresis than in the backgated case.
When devices were appropriately configured, leakage current across the
liquid gate consistently only became significant at gate voltages above
\(\sim\) 0.5 V. Devices which used carbon nanotube films deposited in
surfactant with steam present had significantly less channel-to-channel
variation in electrical characteristics than for devices fabricated
using other approaches. The consistent channel-to-channel electrical
properties of these steam-deposited devices matches the consistent
morphology of the films used to make them, as seen in
Section~\ref{sec-pristine-morphology}. Being able to achieve consistent
subthreshold regime behaviour between channels on the same device is a
...
\includegraphics{./figures/ch5/onoff.png}
...
statistical distribution of (a) the on-off ratio, (b) the threshold
voltage and (c) the subthreshold slope of liquid-gated transistor
channels corresponding to each type of carbon nanotube film deposition.
For each deposition type, measurements were taken from 21 channels of at
least three separate devices. The boxes indicate the 25th and 75th
