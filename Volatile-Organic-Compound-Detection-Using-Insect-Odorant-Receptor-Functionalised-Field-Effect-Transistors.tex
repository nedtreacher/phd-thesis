...
in (b). The current data in (b) has had a moving median filter applied
...
\(I = I_0exp(-t/\tau)\) to each dataset with linear drift subtracted,
where the time constant for the pristine device was
...
at all other times, no flow was placed through the carrier line. A 1000
s recovery period was used between each interval. During each full
sensing series, the temperature remained at 20°C \(\pm\) 2°C. These
sensing series, normalised to the current level directly before the
series, despiked and corrected for drift, are shown side-by-side in
Figure~\ref{fig-E2Hex-sampling} (a). The same dataset after applying a
moving median filter is shown in Figure~\ref{fig-E2Hex-sampling} (b). A
larger current change is shown by the functionalised device than the
pristine device after each addition. However, current responses are not
the sharp, clear responses to analyte of the liquid-gated device seen in
Section~\ref{sec-MeSal-aqueous-sensing}; they appear more similar to the
responses by bare carbon nanotubes to vapour in
\textbf{?@sec-vapour-series}.

There are two potential explanations for the current changes seen in
Figure~\ref{fig-E2Hex-sampling} that do not involve specific detection
of analyte. The functionalisation may have altered the drift behaviour
so that the linear approximation used for removing baseline drift is no
longer valid. The breakdown in this approximation then leads to current
increases that do not directly correspond to analyte being added. The
...
signal-to-noise ratio of the sensor. Regardless, the current increases
observed cannot be described as clear or convincing sensing responses.
Ideally, a working sensor would exhibit clear responses similar to those
seen in Section~\ref{sec-MeSal-aqueous-sensing}.
...
the measurement, a PDMS well containing 50 µL 0.5\% v/v DMSO 1XPBS was
present on the device surface. Significant changes in baseline drift
behaviour are indicated with red dotted lines.}

\end{figure}

It may be necessary to make the sensing environment more similar to that
used for aqueous sensing for reliable sensing; the buffered environment
may help to stabilise the unprotected odorant receptors. A PDMS well can
be attached to the device, and 50 µL 0.5\% v/v DMSO 1XPBS added. Even
though the device is placed into the vapour delivery system upside down,
the hydrophobicity of the PDMS was sufficient to store the buffer
securely in the well. Current sampling over a 1 hour period in this
configuration, with 200 sccm constant flow across the surface of the
well, is presented in Figure~\ref{fig-buffer-vapour}. Gate current
remains stable across the entire measurement period, which indicates
that solution is not leaking out of the PDMS well; otherwise, the
conductive liquid could form a short between the channel electrodes and
backgate, prompting a large increase in gate current. From about 800 s
onwards, fluctuations in the baseline drift become apparent, and at
about 2400 s, the negative drift reverses direction. These undesirable
changes appear to result from the liquid in the well evaporating, the
only significant change in the well environment. When the device chamber
was opened, only \(\sim\) 10-20 µL remained in the well.
...
while eliminating concerns around non-specific binding of the protective
membrane protein. This is the more traditional approach, previously used
for vapour sensing of eugenol with mammalian ORs
...
capture layers can be found in \textbf{?@sec-future-work}.
