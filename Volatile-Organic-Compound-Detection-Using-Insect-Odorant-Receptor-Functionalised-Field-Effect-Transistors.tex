...
developed regions and ensure a clean lift-off.
...
electrodes with a nominal gold layer thickness of 60-100 nm (see
Section~\ref{sec-electrical-characterisation}), and a Au layer nominally
100 nm thick was most commonly used. After metal deposition,
...
\subcaption{\label{fig-old-new-mask}AZ\(^\circledR\) 1518 profile
...
\includegraphics{./figures/ch4/dektat_1518SU8_profile_comparison.png}

}

}

\subcaption{\label{fig-1518-SU8-encapsulation}Comparing photoresist
encapsulation with AZ\(^\circledR\) 1518 and SU8-2150 (using pre-2023
mask)}
\end{minipage}%
...
The exposed channel region was measured using the Dektat profilometer as
shown in Figure~\ref{fig-dektat-encapsulation}. The AZ\(^\circledR\)
1518 exposed region was \(6.8 \pm 0.3\) \(\mu\)m in width when using the
old, pre-2023 mask, while the exposed region was \(16.6 \pm 0.4\)
\(\mu\)m when using AZ\(^\circledR\) 1518 with the new mask from 2023,
as seen in Figure~\ref{fig-old-new-mask}. As noted above, the profile
was more consistent for the new, 2023 mask between devices than for the
old pre-2023 mask.

Compared to AZ\(^\circledR\) 1518, a relatively thin SU-8 encapsulation
layer could be deposited, as seen in
Figure~\ref{fig-1518-SU8-encapsulation}. Using the pre-2023 mask, the
AZ\(^\circledR\) 1518 encapsulation layer had a height of 1.7 \(\pm\)
0.2 \(\mu\)m, while the SU-8 encapsulation layer had a height of 730
\(\pm\) 20 nm. The SU-8 also had less significant edge features than the
AZ\(^\circledR\) 1518. However, the exposed region was reduced for the
SU-8 encapsulation, with an width of only 3.6 \(\pm\) 0.5 \(\mu\)m.
...
to avoid creating other unwanted openings across the surface of the
encapsulation.
...
\caption{\label{fig-dektat-dielectric-layer}Dektat of carbon nanotube
devices after encapsulation photolithography using hardbaked
AZ\(^\circledR\) 1518 and SU-8 2150, taken from various devices.}
...
dielectric metal oxide layer. A electron beam deposition process was
used to deposit a 100-150 nm nominal metal oxide layer on devices
patterned with the 2023 mask using AZ\(^\circledR\) nLOF 2020
photoresist. A silicon dioxide encapsulation layer was tested first.
However, silicon dioxide adheres poorly to gold without an metallic
adhesive layer present, as shown in
Figure~\ref{fig-dielectric-comparison} and as mentioned earlier in
...
\hypertarget{electrodes-only}{%
\subsection{Electrodes only}\label{electrodes-only}}

\hypertarget{encapsulated-devices}{%
\subsection{Encapsulated devices}\label{encapsulated-devices}}
