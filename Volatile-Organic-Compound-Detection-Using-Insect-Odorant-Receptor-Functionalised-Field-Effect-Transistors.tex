...
\frontmatter
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[interior hidden, breakable, boxrule=0pt, frame hidden, borderline west={3pt}{0pt}{shadecolor}, sharp corners, enhanced]}{\end{tcolorbox}}\fi
...
Raman spectroscopy was also used to verify the attachment of
pyrene-group linker molecules to a carbon nanotube network film. The
carbon nanotube films used for verifying pyrene attachment were
solvent-deposited, as highly-bundled devices were found to give a better
optical signal-to-noise ratio, as discussed in
...
to the film which was only exposed to DMSO, indicating surface
modification of the carbon nanotubes. Previous works have found that a
change in the intensity ratio indicates successful \(\pi\)-stacking on
the carbon nanotube surface \autocite{Wei2010,Lan2013}. Wei \emph{et
al.} found functionalisation with PBASE altered the ratio by a factor of
\(\sim 1.5 \times\), an increase, while Lan \emph{et al.} found that
functionalisation with PBA altered the ratio by a factor of
\(\sim 0.8 \times\), a decrease. The reason for the difference between
results is not clear, but may result from the significant differences in
the pristine composition and morphology of carbon nanotube networks used
in each publication. We see from Figure~\ref{fig-raman-comparison} that
across all scan locations, the increase in I\(_D\)/I\(_G\) for both PBA
and PBASE is within the same range. Furthermore, after further
functionalisation of the PBA-modified film with EDC/NHS, subsequent
Raman measurements of the film are not signficantly different to before
adding the EDC/NHS. These results indicate that the
