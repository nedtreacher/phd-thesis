...
volumes of PBS were added during sensing. From Feb 2022 onwards, a 1800
s control series was performed as part of each sensing experiment to
test for unwanted responses to PBS and to allow baseline drift to
settle. 20 \(\mu\)L additions of PBS were added to the PDMS well at 100
s, 200 s and 300 s, and 20 \(\mu\)L of PBS were removed from the PDMS
well at 400 s, 500 s and 600 s. Immediately after the control series, a
sensing sequence was performed as part of the same continuous
measurement set. Most commonly, this would consist of a final PBS
addition at 2100 s, then analyte additions every 300 s after that. The
...
is discussed alongside the relevant experimental results.
...
Hysteresis between the forward and reverse sweep results from charge
traps within and on the surface of the SiO\(_{2}\) dielectric
...
observed, which Kireev \emph{et al.} speculate is due to gate current
having an annealing effect on the channel and removing \(p\)-dopant from
the graphene surface \autocite{Kireev2017}. Kireev \emph{et al.} also
found that continuing to perform liquid-gated sweeps reduced the size of
the second Dirac point, and suggested that it was related removal of
atmospheric contaminants from the graphene surface
\autocite{Kireev2017}. When this occurs, it may be due to the removal of
contaminants improving contact between the metal and graphene surface,
and thus increasing metal doping and consistency of the Fermi level
across the channel.
...
\hypertarget{control-series-and-baseline-drift}{%
\subsection{Control Series and Baseline
Drift}\label{control-series-and-baseline-drift}}
...
\caption{\label{fig-salt-conc-control-series-SU8}These figures show The
linear fits in (b) had R squared values of 0.78 and 0.70 for the traces
with gate voltage \(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-200\) mV and
\(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-150\) mV respectively. The
exponential fits in (c) had R squared values of 0.71 and 0.93 for the
...
to any addition or subtraction of 1XPBS, as expected. Linear fits were
performed on each control series from \(1200-1800\) s. The gradient of
the \(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-200\) mV gated control
series was \(m_{1} = -0.95\pm0.02\) pA/s, while the gradient of the
...
\(b_{2} = (0.73 \pm 0.03)\times m_{1}t + b_{1}\).

When the linear fits were subtracted from the raw data, the remaining
dataset exhibited an exponential decay for each control series.
Figure~\ref{fig-exp-fit} shows exponential fits to the remaining curve
from \(0-1200\) s. Both exponentials have a characteristic time constant
of \(\tau = 300 \pm 20\) s, indicating the rate of decay is independent
of the gate voltage used to gate the transistor. As both exponentials
decay to zero, they are therefore also proportional to each other, with
...
\(-150\) mV measurement. It is possible that the amplitude ratio is
history dependent, where the amplitude of the initial decay curve of the
channel is reduced with each subsequent control series.
...
\caption{\label{fig-salt-conc-control-series-AZ1518}These figures show
The linear fits in (b) had R squared values of 0.96 and for Device 1 and
Device 2 respectively. The exponential fits in (c) had R squared values
of for the \(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-200\) mV and
\(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-150\) mV traces respectively.}
...
device batches. Device 1, with the channel characteristic curve shown in
green, was fabricated in Mar 2023. Device 2, with the channel
characteristic curve shown in purple, was fabricated after Jun 2023.
Device 2 was rinsed with AZ\(^\circledR\) 326 developer and annealed at
...
different channels from a different device batch.

As with the SU8 device, subtracting the linear fit resulted in a a
dataset which could be fitted with an exponential function.
Figure~\ref{fig-exp-fit-AZ1518} shows exponential fits from \(0-1200\) s
to the AZ\(^\circledR\) 1518 devices. Both exponentials have a
characteristic time constant of \(\tau = 300 \pm 20\) s, indicating the
rate of decay is independent of the gate voltage used to gate the
transistor. As both exponentials decay to zero, they are therefore also
proportional to each other, with
\(a_{2}\exp(-t/\tau) = (2.39\pm0.05)\times a_{1}\exp(-t/\tau)\). The
\(V_{\textrm{lg}}-V_{\textrm{gap}}\) = \(-200\) mV measurement was
performed 3 days after the \(V_{\textrm{lg}}-V_{\textrm{gap}}\) =
\(-150\) mV measurement. It is possible that the amplitude ratio is
...
liquid-gated carbon nanotube devices can be modelled as a combination of
a exponential, linear and constant term. The linear term appears to be
consistent between channels of a particular device type,
...
setups. The code consists of three related but independent modules: the
first analyses and plots sensing data from the FET devices, the second
