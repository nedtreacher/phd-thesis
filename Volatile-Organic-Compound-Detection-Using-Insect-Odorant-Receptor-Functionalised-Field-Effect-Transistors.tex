...
a Karl Suss MJB3 Contact Aligner with a USHIO SHP 350 W lamp (USH-350DS,
Japan). When performing lithography, the intensity reading from the
aligner was 20.8 - 22.6 mW/cm\(^2\) (Note however that an external
photometer reading at 400 nm found an intensity output of 17.2
mW/cm\(^2\) when the aligner read 21.0 mW/cm\(^2\)). The unexposed areas
of the negative resist are then removed in the development process
\autocite{Microchem}. Figure~\ref{fig-photolithography-types} gives a
...
The step-by-step processes for each resist are detailed here in the
subsequent sections.
...
\item
  Spincoat at 4000 rotations per minute (rpm) for 1 minute (note: use
  the minimum amount of photoresist required to fully cover the wafer
  surface)
\item
