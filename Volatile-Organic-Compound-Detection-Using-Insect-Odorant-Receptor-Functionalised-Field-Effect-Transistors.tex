...
chamber. The 10 mL analyte Schott bottle (Duran) is shown connected to
the carrier line in Figure~\ref{fig-analyte-bottle}. Each line consist
of a mix of stainless steel and flexible PVC tubing, with various
Swagelok fittings and valves. These valves include check valves, to
ensure there is no backflow of vapour mass within the system. The
...
pass by the relative humidity indicator (P = 0 sccm and exhaust goes to
RHI in Figure~\ref{fig-chamber-schematic}). Continuous nitrogen flow was
then placed through the chamber until relative humidity dropped to about
20\%. 10 mL of deionised water was placed into the analyte bottle. The
following sequence of flows shown in Table~\ref{tbl-RHI-flow-sequence}
was placed through the chamber (t = time, C = carrier line flow rate, D
= dilution line flow rate). Note that between 230 s and 630 s, the total
flow rate remains the same, but the ratio of dilution to carrier flow
differs.

From Figure~\ref{fig-RHI-verification}, it is clear that with increased
flow from the analyte bottle containing deionised water, the relative
humidity registered by the Telaire sensor also increases. The increases
in relative humidity occur about 50 s after corresponding increases in
flow through the carrier line, which appears to be the time taken for
water vapour to first reach the relative humidity indicator chamber.
Over the full 800 s of carrier line flow, relative humidity increases
from a minimum of 14.0\% to a maximum of 44.3\%.
