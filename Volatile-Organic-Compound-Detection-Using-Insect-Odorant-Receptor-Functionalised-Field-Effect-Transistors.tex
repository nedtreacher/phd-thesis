...
\hypertarget{interactions-between-substrate-and-pyrene-peg}{%
\subsubsection*{Interactions between Substrate and
Pyrene-PEG}\label{interactions-between-substrate-and-pyrene-peg}}
...
\hypertarget{functionalisation-with-gfp-or-nanodiscs-using-pbase}{%
\subsection{Functionalisation with GFP-OR Nanodiscs using
PBASE}\label{functionalisation-with-gfp-or-nanodiscs-using-pbase}}
...
nanodiscs. Images were taken using an GFP filter and a 10 s exposure
time, where (a) is channel 1 of the device, (b) is channel 2, (c) is
channel 3 and (d) is channel 6.}
...
\item
  1 mM PBASE (Setareh Biotech) in methanol prepared by fully dissolving
  2 mg PBASE in 5 mL methanol by vortex mixing at 1000 rpm in a dark
  room (Note: PBASE was stored at -4\(^\circ\)C for 18 months prior to
  use, and was thawed under vacuum for 15 minutes in dark conditions
  before opening)
\item
  ...
  1 hour, then rinsed with methanol for 15 s, rinsed with DI water for
  15 s and nitrogen dried to remove residual PBASE.
\item
...
shown in Figure~\ref{fig-PBASE-GFP-ORs}.

The dark regions are the gold electrodes, and the bright green region is
the graphene channel. It is noticable that some variation in
fluorescence is seen between channels, indicating the quality of
functionalisation may vary across the channels of a device. Some of the
difficulties encountered when investigating functionalisation with
pyrene-PEG-FITC are outlined below.

The reasons underlying the steps used for the PBASE functionalisation
...
\hypertarget{photoresist-contamination-1}{%
\subsubsection*{Photoresist
Contamination}\label{photoresist-contamination-1}}
\addcontentsline{toc}{subsubsection}{Photoresist Contamination}

\hypertarget{marangoni-flow-and-coffee-ring-effect}{%
\subsubsection*{Marangoni Flow and Coffee-Ring
Effect}\label{marangoni-flow-and-coffee-ring-effect}}
\addcontentsline{toc}{subsubsection}{Marangoni Flow and Coffee-Ring
Effect}
...
Verifies that ORs are attaching, not just nanodiscs (empty nanodiscs
also have NH2 group)

\hypertarget{interactions-between-substrate-and-pyrene-peg-1}{%
\subsubsection*{Interactions between Substrate and
Pyrene-PEG}\label{interactions-between-substrate-and-pyrene-peg-1}}
\addcontentsline{toc}{subsubsection}{Interactions between Substrate and
Pyrene-PEG}
