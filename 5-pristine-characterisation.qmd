...
From this analysis it appears that the baseline drift for the liquid-gated carbon nanotube devices can be accurately approximated as a combination of a exponential, linear and constant term. The linear term appears to be consistent across channels of a particular device type, with the size of this term increasing with increasingly negative gate bias. The time constant of the exponential term appears to be intrinsic to the channel being measured. 
...
Intriguingly, even though the largest change in PBS concentration occurred at the first deionised water addition (see @tbl-salt-conc-series), there was very little signal change across all channels, while a relatively large change occurred at the second addition. The threshold shift due to electrostatic gating should be proportional to the logarithm of final salt concentration, and therefore proportional to signal change in the linear on-regime [@Heller2010]. @fig-salt-conc-box-plot shows the signal change in terms of this logarithmic relationship. We see that the median values of the first two additions do not line up well with the overall logarithmic trend. Insufficient mixing with existing solution of a specific addition in the tightly enclosed PDMS well environment may be responsible for this result. Subsequent additions may be more likely to cause mixing, leading to the change in concentration at the surface of the channel being more similar to the change in concentration in the bulk of the solution in the well.
...
If normalised so that the size of signal change is measured relative to the final instead of initial current, (b) and (c) appear as shown in (e) and (f) respectively.
:::


(e) and (f) account for the difference in drift between channels, show they are consistent when drift accounted for

#### Signal-to-Noise Ratio {.unnumbered}

::: {#fig-salt-conc-SNR layout="[[-25,50,-25]]"}
