...
from multilayer coverage restricting access by the odorant receptors to
...
attach OR22a nanodiscs. The concentration of each 20 µL addition is
...
\caption{\label{fig-DMSO-TX}Fiquid-gated carbon nanotube network device
transfer characteristics before and after OR22a nanodisc
...
threshold height of 12.3 nm is shown in (b). The cross-sections in (c)
and (d) demonstrate that the long, rounded features seen in (b)
correspond to densely packed collections of nanodisc aggregates
following the length of a carbon nanotube.}

\end{figure}

showing long, rounded features with a \emph{Hormosira}-like appearance
\autocite{NewZealandPlantConservationNetwork}.

-DMSO promoting hydrolysis PBA/EDC

Remove the possibility of contamination with solvent or PBASE multilayer
coverage
...
\hypertarget{aqueous-based-functionalisation-with-or35a}{%
\subsection{Aqueous-Based Functionalisation with
OR35a}\label{aqueous-based-functionalisation-with-or35a}}
