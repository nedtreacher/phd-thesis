...
\includegraphics{figures/ch6/GFPOR_Ni2+_DFQ3C3_20sexposure_40X_mediumcontrast_ch6_231018.png}

}

}

\subcaption{\label{fig-OR-GFP-surfactant-1}}
\end{minipage}%
%
\begin{minipage}[t]{0.05\linewidth}
...
\includegraphics{figures/ch6/GFPOR_Ni2+_DFQ3C3_20sexposure_40X_mediumcontrast_ch7_231018.png}

}

}

\subcaption{\label{fig-OR-GFP-surfactant-2}}
\end{minipage}%
...
\(\mu\)L/mL GFP-OR43b nanodiscs in 1XPBS using pyrene-PEG-NTA. Images
(c) and (d) also shows channels functionalised with 1 \(\mu\)L/mL
GFP-OR43b, but the device was rinsed with surfactant directly after
pyrene-PEG-NTA attachment and before metal ion attachment. Image (c) is
device channel 6 and image (d) is device channel 7. Images (a) and (b)
were taken using a 10 s exposure time, while (c) and (d) were taken
using a 20 s exposure time. (b)-(d) have been brightened by 40\% for
visual clarity. All images were taken using a GFP filter.}
...
Surfactant rinsing was used to verify the specific attachment of
GFP-OR43b nanodiscs to the Pyrene-PEG-NTA, in the same manner as
outlined in Section~\ref{sec-PBASE-GFP-OR-attachment}.
Figure~\ref{fig-OR-GFP-surfactant-1}-d show fluorescence microscope
images of the device after functionalisation with GFP-OR43b. We see that
the fluorescent protein is predominantly present on the channel surface,
indicating that OR-GFP43b is only attaching to linker on the channel
region. This indicates that the surfactant rinse removed pyrene-PEG-NTA
from everywhere except the channels, due to its relatively strong
\(\pi\)-stacking interaction with the carbon nanotubes.
