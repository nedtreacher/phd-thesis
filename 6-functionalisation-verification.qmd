...
Alongside PBASE, there are a wide range of other pyrene-based linker molecules used for modification of carbon nanotubes and graphene. A few of these linkers are examined further below.
...
Through chemical coupling/conjugation, it is possible to replace the NHS ester group on PBASE with other groups that can undergo binding reactions with proteins. For example, PBASE can be modified with N$\alpha$,N$\alpha$-Bis(carboxymethyl)-L-lysine hydrate (also known as N-(5-Amino-1-carboxypentyl)iminodiacetic acid, AB-NTA) to produce pyrene-nitrilotriacetic acid (pyrene-NTA). The attached NTA group is able to chelate with metal ions such as Cu$^{2+}$ or Ni$^{2+}$, which then can then coordinate with polyhistidine-tags attached to a protein [@Holzinger2011; @Amano2016]. This functionalisation procedure was successfully used with mammalian odorant receptors attached via Ni$^{2+}$ to a single carbon nanotube to detect eugenol vapour in real-time [@Goldsmith2011]. Pyrene-biotin (pyrene butanol biotin ester) can also be produced for attaching avidin, ....

It is also possible to attach polyethylene glycol (PEG) chains to a pyrene group and modify them with reactive groups such as biotin to attach proteins for biosensing [@Hermanson2013-18; @Meran2018]. Once modified with PEG, pyrene linkers are also more water soluble, making it possible to perform a full functionalisation procedure exclusively in aqueous solution [@Hermanson2013-18]. 
