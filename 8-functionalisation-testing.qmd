...
A normalised sensing series across four multiplexed channels from a device functionalised with PBASE in DMSO to attach OR22a nanodiscs. The concentration of each 20 µL addition is indicated above the time of addition. Current data has been despiked and baseline drift removed in both (a) and (b), while a moving median filter has also been applied to the dataset shown in (b). 
:::
...
Fiquid-gated carbon nanotube network device transfer characteristics before and after OR22a nanodisc functionalisation using PBASE in DMSO. Each subfigure (a)-(d) corresponds to a different channel of the functionalised device; (a) corresponds to channel 1, (b) corresponds to channel 2, (c) corresponds to channel 3, and (d) corresponds to channel 4. The dashed line shown is tangent to the subthreshold slope of the characteristic curve. The threshold voltage associated with each curve is also shown.
:::
...
Atomic force microscope images of a carbon nanotube film functionalised with OR22a nanodiscs using PBASE in DMSO. The average substrate height is shown in green in (a). A binary representation of the atomic force microscope image with a threshold height of 12.3 nm is shown in (b). The cross-sections in (c) and (d) demonstrate that the long, rounded features seen in (b) correspond to densely packed collections of nanodisc aggregates following the length of a carbon nanotube.
:::



showing long, rounded features with a *Hormosira*-like appearance [@NewZealandPlantConservationNetwork].

-DMSO promoting hydrolysis PBA/EDC

Remove the possibility of contamination with solvent or PBASE multilayer coverage
